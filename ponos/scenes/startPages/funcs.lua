...
function M.create(group, params)
	...
	local panel = PonosUi.newBottomBar(function(index)
            if index == "next" then
			    timer.cancelAll()
                scene.goto("startPages.blocks" , { transition = "slideLeft" } )
            end
        end,{
		    {},
    ...
    local elementsTable = {
        {type = "title", text = app.words[250]},
        {type = "text", text = app.words[251]},
		{type = "text", text = app.words[252]},
        {type = "image-swift", list = {
            "res/unknow/screenshot1.jpeg",
            "res/unknow/screenshot2.jpg",
			"res/unknow/screenshot5.png",
            "res/unknow/screenshot6.jpg",
        }},
		{type = "text", text = app.words[253]},
    }
    ...
    local function createElement()
        ...
        elseif data.type == "image-swift" then
            ...
            local currentImage = display.newImageRect(elementGroup, data.list[picIndex], 800, 480)
            ...
            local function nextImage()
                ...
                local newImage = display.newImageRect(elementGroup, data.list[picIndex], 800, 480)
                if newImage then
